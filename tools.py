...
def plot_curves(values_dict,
    ...
    :param dpi: int, the dpi of the image.
    """
    msg = "'values' must be dict. You provided `{}` .... [NOT OK]".format(type(values_dict))
    assert isinstance(values_dict, dict), msg

    for key, value in values_dict.items():
        if key not in ['total_loss','loss_pos', 'loss_neg']:
            values_dict[key] = [x.cpu().item() for x in value]
