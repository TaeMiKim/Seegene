...
class Recall(nn.Module):
    ...
    def forward(self, pred_m, true_m):
        ...
        recall = (intersection + self.smooth) / (tflat.sum(dim=1) + self.smooth)
...
class Precision(nn.Module):
    ...
    def forward(self, pred_m, true_m):
        ...
        precision = (intersection + self.smooth) / (pflat.sum(dim=1) + self.smooth)
...
class Metrics(nn.Module):
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            recall = self.recall(pred_m=ppixels,
                                true_m=masks_trg
                                ).sum()
            
            precision = self.precision(pred_m=ppixels,
                                true_m=masks_trg
                                ).sum()
        ...
        return acc, dice_forg, dice_back, iou, recall, precision
